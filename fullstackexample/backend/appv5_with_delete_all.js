...
const server = http.createServer(async (req, res) => {
    ...
    try {
        // --- Normal Base Return for the root path (GET /) ---
        ...
        // --- NEW ROUTE: 4. DELETE ALL (DELETE /messages) ---
        else if (req.method === 'DELETE' && req.url === '/messages') {
            pool.query('TRUNCATE TABLE messages', (err, result) => {
                if (err) throw err;
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ message: 'All records deleted successfully, table reset.' }));
            });
        }

        // --- 5. DELETE BY ID (DELETE /messages/id) ---
